...
function Deck(options) {
  ...
  assert.equal(typeof this.options.cards, 'number',
               'options.cards is required (number)');
  assert(this.options.cards <= 0xff, 'Can\'t have more than 255 cards');

  this.curve = this.options.curve;
  this.cards = [];

  this._shuffle = new Shuffle(this.options.cards);
  ...
  this._drawn = [];
  for (var i = 0; i < this.options.cards; i++) {
