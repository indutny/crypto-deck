...
Protocol.prototype._mutex = function _mutex(body, callback) {
  if (this.index === FIRST_PLAYER)
...
Protocol.prototype._localMutex = function _localMutex(from, body, callback) {
...
Protocol.prototype._remoteMutex = function _remoteMutex(body, callback) {
  if (this._mutexHolder !== null) {
...
Protocol.prototype._listen = function _listen() {
  ...
  var draw = this._stream('draw');
  draw.on('data', function(item) {
    self.recvDraw(item.payload, item.from);
  });

  var open = this._stream('open');
  open.on('data', function(item) {
    self.recvOpen(item.payload, item.from);
  });

  var update = this._stream('update');
  update.on('data', function(item) {
