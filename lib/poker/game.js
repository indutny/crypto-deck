...
Game.prototype.recvIdleMsg = function recvIdleMsg(msg, from) {
  if (msg.type === 'draw')
    ...
    return this.recvUpdate(msg, from);
  else
    return this._error('unexpected message: ' + msg.type, from);
};
...
Game.prototype.update = function update(info) {
  var self = this;
  if (this.state !== 'idle') {
    this._idleQueue.push(function() {
      self.open(index, callback);
...
Game.prototype.recvUpdate = function recvUpdate(msg, from) {
  try {
    this.controller.validateUpdate(msg.payload, from);
  } catch (e) {
    return this._error('Failed to update game state', from);
  }
